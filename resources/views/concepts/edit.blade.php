@extends('lte.root')

@push('pageTitle', 'Conceptos | Agregar')

@section('content')
    <div class="row">
        <div class="col-md-6">
            <color-box title="Agregar nuevo concepto" color="vks">

                {!! Form::open(['method' => 'POST', 'route' => ['concepts.update', $concept]]) !!}

                    {!! Field::select('provider_id', $providers, $concept->provider_id,
                        ['empty' => 'Seleccione el proveedor', 'tpl' => 'lte/withicon'], ['icon' => 'truck'])
                    !!}

                    {!! Field::text('description', $concept->description, ['tpl' => 'lte/withicon'], ['icon' => 'comments']) !!}

                    {!! Form::submit('Agregar', ['class' => 'btn btn-github pull-right']) !!}

                {!! Form::close() !!}
            </color-box>
        </div>
        
    </div>
@endsection